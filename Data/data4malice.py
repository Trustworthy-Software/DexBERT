...
if __name__ == '__main__':
    ...
    sample_list_file_path = 'data/malicious_code_localization.txt'
    malware_apk_dir      = 'data/malicious_apks'  # put the downloaded apks in this dir
    malware_txt_dir      = 'data/malicious_txts'  # the processed txt file will be generated and saved in this dir
    
    for sample in tqdm(open(sample_list_file_path, 'r').readlines()):
        hash = sample.strip()
        apk_path = osp.join(malware_apk_dir, hash)
        smali_dir = osp.join(malware_apk_dir, hash.split('.')[0])
        Disassemble(apk_path, smali_dir)
        Smalis2Txt(malware_txt_dir, smali_dir, only_keep_func_name=False)
